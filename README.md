...
> **Status: Phase 2 of 5 complete.** Classic mode is playable: a crisp
> interpolated body, a glowing pulse of food, and a death that flashes, shakes,
> and scatters the whole snake into particles. Endless and Daily
...
- **Glow is rationed.** It accumulates into a half-resolution buffer that gets
  a separable Gaussian blur before compositing - additive brightness with no
  spatial spread is not glow, just a brighter pixel. Only the food and the death
  burst use it. The snake itself stays crisp, so when something does glow it
  means something.
